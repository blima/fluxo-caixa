...
import { lancamentosApi, destinosApi, etiquetasApi, tiposPagamentoApi } from '@/services/api';
import { Lancamento, Destino, Etiqueta, TipoPagamento } from '@/types';
...
export default function DespesasPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await lancamentosApi.list({ tipo: 'despesa' });
      setItems(res.data);
    } catch {
  ...
  const loadAux = async () => {
    const [d, e, tp] = await Promise.all([
      destinosApi.list(),
      etiquetasApi.list(),
    ...
    return { destinos: d.data, etiquetas: e.data };
  };
  ...
  const openCreate = async () => {
    ...
    const destinoPadrao = aux.destinos.find((d: Destino) => d.padrao);
    setDestinoId(destinoPadrao?.id || aux.destinos[0]?.id || '');
    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        {loading ? (
          ...
          <EmptyState message="Nenhuma despesa cadastrada" action={
